...
import fui_cli.commands.build
import fui_cli.commands.create
import fui_cli.commands.pack
import fui_cli.commands.publish
import fui_cli.commands.run
...
def main():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=fui.version.version if fui.version.version else update_version(),
    )
    sp = parser.add_subparsers(dest="command")
    # sp.default = "run"

    fui_cli.commands.create.Command.register_to(sp, "create")
    fui_cli.commands.run.Command.register_to(sp, "run")
    fui_cli.commands.pack.Command.register_to(sp, "pack")
    fui_cli.commands.publish.Command.register_to(sp, "publish")
    fui_cli.commands.build.Command.register_to(sp, "build")
