...
from fui_cli.commands.base import BaseCommand
from fui_cli.utils.merge import merge_dict
from fui_cli.utils.project_dependencies import (
    get_poetry_dependencies,
    get_project_dependencies,
)
from fui_cli.utils.pyproject_toml import load_pyproject_toml
...
DEFAULT_TEMPLATE_URL = "gh:fui-dev/fui-build-template"
...
class Command(BaseCommand):
    ...
    def __init__(self, parser: argparse.ArgumentParser) -> None:
        ...
        self.pck_name = {
            "ads":"fui_ads",
            "audio":"fui_audio",
            "audio_recorder":"fui_audio_recorder",
            "camera":"fui_camera",
            "flashlight":"fui_flashlight",
            "geolocator":"fui_geolocator",
            "lightsensor":"fui_lightsensor",
            "lottie":"fui_lottie",
            "map":"fui_map",
            "permission_handler":"fui_permission_handler",
            "rive":"fui_rive",
            "video":"fui_video",
            "webview":"webview"
        }
        self.skip_flutter_doctor = get_bool_env_var("FUI_CLI_SKIP_FLUTTER_DOCTOR")
        self.no_rich_output = get_bool_env_var("FUI_CLI_NO_RICH_OUTPUT")
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            nargs="+",
            default=[],
            help="include extra Flutter fui packages, such as fui_video, fui_audio, etc.",
        )
        parser.add_argument(
            "--info-plist",
            dest="info_plist",
    ...
    def validate_entry_point(self):
        ...
        self.python_module_name = Path(
            ...
            or cast(str, self.get_pyproject("tool.fui.app.module"))
            or "main"
        ).stem
    ...
    def setup_template_data(
        ...
        base_url = (
            (
                ...
                or cast(str, self.get_pyproject("tool.fui.web.base_url"))
                or "/"
            )
            .strip("/")
            .strip()
        ...
        product_name = (
            ...
            or self.get_pyproject("tool.fui.product")
        ...
        split_per_abi = (
            ...
            if self.options.split_per_abi is not None
                ...
                self.get_pyproject("tool.fui.android.split_per_abi")
                if self.get_pyproject("tool.fui.android.split_per_abi") is not None
                else False
            )
        )
        ...
        for p in (
            ...
            or self.get_pyproject("tool.fui.permissions")
            or []
        ):
        ...
        info_plist = merge_dict(
            info_plist,
            (
                self.get_pyproject("tool.fui.macos.info")
                if self.package_platform == "Darwin"
                else self.get_pyproject("tool.fui.ios.info")
            )
            or {},
        )
        ...
        macos_entitlements = merge_dict(
            macos_entitlements,
            self.get_pyproject("tool.fui.macos.entitlement") or {},
        )
        ...
        android_permissions = merge_dict(
            android_permissions,
            self.get_pyproject("tool.fui.android.permission") or {},
        )
        ...
        android_features = merge_dict(
            android_features,
            self.get_pyproject("tool.fui.android.feature") or {},
        )
        ...
        android_meta_data = merge_dict(
            android_meta_data,
            self.get_pyproject("tool.fui.android.meta_data") or {},
        )
        ...
        deep_linking_scheme = (
            self.get_pyproject("tool.fui.ios.deep_linking.scheme")
            if self.package_platform == "iOS"
            else (
                self.get_pyproject("tool.fui.android.deep_linking.scheme")
                if self.package_platform == "Android"
                else self.get_pyproject("tool.fui.deep_linking.scheme")
            )
        )

        deep_linking_host = (
            self.get_pyproject("tool.fui.ios.deep_linking.host")
            if self.package_platform == "iOS"
            else (
                self.get_pyproject("tool.fui.android.deep_linking.host")
                if self.package_platform == "Android"
                else self.get_pyproject("tool.fui.deep_linking.host")
            )
        )
        ...
        self.template_data = {
            ...
            "route_url_strategy": (
                ...
                or self.get_pyproject("tool.fui.web.route_url_strategy")
                or "path"
            ),
            "web_renderer": (
                self.options.web_renderer
                or self.get_pyproject("tool.fui.web.renderer")
                or "canvaskit"
            ),
            "use_color_emoji": (
                "true"
                if self.options.use_color_emoji
                or self.get_pyproject("tool.fui.web.use_color_emoji")
                else "false"
            ),
            "pwa_background_color": (
                self.options.pwa_background_color
                or self.get_pyproject("tool.fui.web.pwa_background_color")
            ),
            "pwa_theme_color": (
                self.options.pwa_theme_color
                or self.get_pyproject("tool.fui.web.pwa_theme_color")
            ),
            ...
            "org_name": self.options.org_name or self.get_pyproject("tool.fui.org"),
            "company_name": (
                self.options.company_name or self.get_pyproject("tool.fui.company")
            ),
            "copyright": self.options.copyright
            or self.get_pyproject("tool.fui.copyright"),
            "team_id": self.options.team_id or self.get_pyproject("tool.fui.ios.team"),
            "options": {
    ...
    def get_flutter_dependencies(self) -> dict:
        ...
        flutter_dependencies = (
            self.get_pyproject("tool.fui.flutter.dependencies") or {}
        )
    ...
    def create_flutter_project(self):
        ...
        template_url = (
            ...
            or self.get_pyproject("tool.fui.template.url")
            or DEFAULT_TEMPLATE_URL
        )
        template_ref = self.options.template_ref or self.get_pyproject(
            "tool.fui.template.ref"
        )
        if not template_ref:
            template_ref = (
                version.Version(fui.version.version).base_version
                if fui.version.version
                else update_version()
            )
        template_dir = self.options.template_dir or self.get_pyproject(
            "tool.fui.template.dir"
        )
    ...
    def load_pubspec(self):
        ...
        # add local fui packages to a dest pubspec.yaml
        self.status.update(
            f"[bold blue]Adding Local Fui Packages To Project {self.emojis['loading']}... "
        )
        self.fui_packages_path = str(Path(__file__).parent.joinpath("packages"))
        if not os.path.exists(self.fui_packages_path):
            console.log(
                f"Local Fui Packages Not Found!"
            )
            console.log(
        ...
        console.log(
            f"Local Fui Packages Added To Project {self.emojis['checkmark']}"
        )

        self.pubspec = merge_dict(
            self.pubspec, self.get_pyproject("tool.fui.flutter.pubspec") or {}
        )
    ...
    def customize_icons_and_splash_images(self):
        ...
        adaptive_icon_background = (
            ...
            or self.get_pyproject("tool.fui.android.adaptive_icon_background")
        )
        ...
        self.pubspec["flutter_native_splash"]["web"] = (
            ...
            if self.options.no_web_splash is not None
                ...
                self.get_pyproject("tool.fui.splash.web")
                if self.get_pyproject("tool.fui.splash.web") is not None
                else True
            )
        )
        self.pubspec["flutter_native_splash"]["ios"] = (
            ...
            if self.options.no_ios_splash is not None
                ...
                self.get_pyproject("tool.fui.splash.ios")
                if self.get_pyproject("tool.fui.splash.ios") is not None
                else True
            )
        )
        self.pubspec["flutter_native_splash"]["android"] = (
            ...
            if self.options.no_android_splash is not None
                ...
                self.get_pyproject("tool.fui.splash.android")
                if self.get_pyproject("tool.fui.splash.android") is not None
                else True
            )
        )
    ...
    def package_python_app(self):
        ...
        target_arch = (
            ...
            or self.get_pyproject(f"tool.fui.{self.config_platform}.build_arch")
            or self.get_pyproject("tool.fui.build_arch")
        )
        if target_arch:
        ...
        if (
            ...
            if self.options.compile_app is not None
                ...
                self.get_pyproject("tool.fui.compile.app")
        ...
        if (
            ...
            if self.options.compile_packages is not None
                ...
                self.get_pyproject("tool.fui.compile.packages")
        ...
        if (
            ...
            if self.options.cleanup_on_compile is not None
                ...
                self.get_pyproject("tool.fui.compile.cleanup")
        ...
        if not os.path.exists(app_zip_path):
            self.cleanup(1, "fui app package app/app.zip was not created.")
    ...
    def flutter_build(self):
        ...
        android_signing_key_store = (
            ...
            or self.get_pyproject("tool.fui.android.signing.key_store")
        )
        if android_signing_key_store:
            build_env["FUI_ANDROID_SIGNING_KEY_STORE"] = android_signing_key_store

        key_store_password = (
            self.options.android_signing_key_store_password
            or os.getenv("FUI_ANDROID_SIGNING_KEY_STORE_PASSWORD")
        )
        key_password = self.options.android_signing_key_password or os.getenv(
            "FUI_ANDROID_SIGNING_KEY_PASSWORD"
        )
        if key_store_password or key_password:
            build_env["FUI_ANDROID_SIGNING_KEY_STORE_PASSWORD"] = (
                key_store_password if key_store_password else key_password
            )
            build_env["FUI_ANDROID_SIGNING_KEY_PASSWORD"] = (
                key_password if key_password else key_store_password
            )

        android_signing_key_alias = (
            self.options.android_signing_key_alias
            or self.get_pyproject("tool.fui.android.signing.key_alias")
        )
        if android_signing_key_alias:
            build_env["FUI_ANDROID_SIGNING_KEY_ALIAS"] = android_signing_key_alias
        ...
        for arg in self.get_pyproject("tool.fui.flutter.build_args") or []:
    ...
    def cleanup(
        ...
        if exit_code == 0:
            ...
            msg = (
                ...
                else "Error building fui app - see the log of failed command above."
            )
                    ...
                    if match:
                        ...
                        if flutter_version < version.parse(MINIMAL_FLUTTER_VERSION):
                            flutter_msg = (
                                ...
                                + f"fui build requires Flutter {MINIMAL_FLUTTER_VERSION} or above. "
    ...
    def get_fui_extensions_imports(self,file_path):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("fui.extensions"):
                        im_name = alias.name.replace("fui.extensions.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
            elif isinstance(node, ast.ImportFrom):
                if node.module and node.module.startswith("fui.extensions"):
                    for alias in node.names:
                        full_import = f"{node.module}.{alias.name}"
                        im_name = full_import.replace("fui.extensions.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("fui.core"):
                        im_name = alias.name.replace("fui.core.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
            elif isinstance(node, ast.ImportFrom):
                if node.module and node.module.startswith("fui.core"):
                    for alias in node.names:
                        full_import = f"{node.module}.{alias.name}"
                        im_name = full_import.replace("fui.core.","")
                        if im_name in self.pck_name.keys():
                            imported_items.append(self.pck_name[im_name])
