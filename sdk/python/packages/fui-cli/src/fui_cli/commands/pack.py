...
from fui.utils import is_macos, is_windows
from fui_cli.commands.base import BaseCommand


class Command(BaseCommand):
    """
    Package fui app to a standalone bundle.
    """
    ...
    def handle(self, options: argparse.Namespace) -> None:
        from fui.utils.pip import ensure_fui_desktop_package_installed
        ...
        try:
            ...
            from fui_cli.__pyinstaller.utils import copy_fui_bin
            ...
            # copy "bin"
            hook_config.temp_bin_dir = copy_fui_bin()

            if hook_config.temp_bin_dir is not None:
                # delete fuid/fuid.exe
                fuid_path = os.path.join(
                    hook_config.temp_bin_dir, "fuid.exe" if is_windows() else "fuid"
                )
                if os.path.exists(fuid_path):
                    os.remove(fuid_path)

                if is_windows():
                    from fui_cli.__pyinstaller.win_utils import (
                        update_fui_view_icon,
                        update_fui_view_version_info,
                    )

                    exe_path = os.path.join(
                        hook_config.temp_bin_dir, "fui", "fui.exe"
                    )
                    if os.path.exists(exe_path):
                        ...
                        if options.icon:
                            ...
                            update_fui_view_icon(exe_path, icon_path)

                        # version info
                        version_info_path = update_fui_view_version_info(
                            exe_path=exe_path,
                    ...
                    from fui_cli.__pyinstaller.macos_utils import (
                        assemble_app_bundle,
                        unpack_app_bundle,
                        update_fui_view_icon,
                        update_fui_view_version_info,
                    ...
                    tar_path = os.path.join(
                        hook_config.temp_bin_dir, "fui-macos-amd64.tar.gz"
                    )
                    if os.path.exists(tar_path):
                        ...
                        if options.icon:
                            ...
                            if not Path(icon_path).is_absolute():
                                icon_path = str(Path(os.getcwd()).joinpath(icon_path))
                            update_fui_view_icon(app_path, icon_path)

                        # version info
                        app_path = update_fui_view_version_info(
                            app_path=app_path,
            ...
            print("Please install PyInstaller module to use fui pack command:", e)
            sys.exit(1)
