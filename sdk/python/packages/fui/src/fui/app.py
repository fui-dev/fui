...
import fui.version
from fui.core.event import Event
from fui.core.page import Page
from fui.core.types import AppView, WebRenderer
...
def app(
    ...
    host=None,
    port=0,
    view: Optional[AppView] = AppView.fui_APP,
    assets_dir="assets",
    upload_dir=None,
    ...
    if export_asgi_app:
        ensure_fui_web_package_installed()
        from fui_web.fastapi.serve_fastapi_web_app import get_fastapi_web_app
...
async def app_async(
    ...
    force_web_server = get_bool_env_var("FUI_FORCE_WEB_SERVER") or is_linux_server()
    if force_web_server:
        view = AppView.WEB_BROWSER

    env_port = os.getenv("FUI_SERVER_PORT")
    ...
    env_host = os.getenv("FUI_SERVER_IP")
    ...
    is_socket_server = (
        is_embedded() or view == AppView.fui_APP or view == AppView.fui_APP_HIDDEN
    ) and not force_web_server

    url_prefix = os.getenv("FUI_DISPLAY_URL_PREFIX")
    ...
    def exit_gracefully(signum, frame):
        logger.debug("Gracefully terminating fui app...")
    ...
    logger.info("fui app has started...")

    try:
        if (
            (
                view == AppView.fui_APP
                or view == AppView.fui_APP_HIDDEN
                or view == AppView.fui_APP_WEB
            )
            and not force_web_server
            and not is_embedded()
            ...
            ensure_fui_desktop_package_installed()
            from fui_desktop import close_fui_view, open_fui_view_async

            on_app_startup(conn.page_url)

            fvp, pid_file = await open_fui_view_async(
                conn.page_url,
                assets_dir if view != AppView.fui_APP_WEB else None,
                view == AppView.fui_APP_HIDDEN,
            )
            try:
                await fvp.wait()
            except:
...
async def __run_socket_server(port=0, session_handler=None, blocking=False):
    from fui.fui_socket_server import fuiSocketServer

    uds_path = os.getenv("FUI_SERVER_UDS_PATH")
...
async def __run_web_server(
    ...
    ensure_fui_web_package_installed()
    from fui_web.fastapi.serve_fastapi_web_app import serve_fastapi_web_app
    ...
    logger.info(f"Starting fui web server on port {port}...")

    log_level = logging.getLogger(fui.__name__).getEffectiveLevel()
...
def __run_pyodide(target):
    import fui_js
    from fui.pyodide_connection import PyodideConnection
...
def __get_page_name(name: str):
    env_page_name = os.getenv("FUI_WEB_APP_PATH")
...
def __get_assets_dir_path(assets_dir: Optional[str], relative_to_cwd=False):
    ...
    env_assets_dir = os.getenv("FUI_ASSETS_DIR")
    if env_assets_dir:
