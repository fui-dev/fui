...
import fui_web.fastapi as fui_fastapi
from fastapi import WebSocket, WebSocketDisconnect
from fui.core.event import Event
from fui.core.local_connection import LocalConnection
from fui.core.page import Page, PageDisconnectedException
from fui.core.protocol import (
    ClientActions,
    ClientMessage,
    Command,
    CommandEncoder,
...
from fui.utils import random_string, sha1
from fui_web.fastapi.fui_app_manager import app_manager
from fui_web.fastapi.oauth_state import OAuthState
from fui_web.uploads import build_upload_url

logger = logging.getLogger(fui_fastapi.__name__)

DEFAULT_fui_SESSION_TIMEOUT = 3600
DEFAULT_fui_OAUTH_STATE_TIMEOUT = 600


class fuiApp(LocalConnection):
    def __init__(
        self,
        loop: asyncio.AbstractEventLoop,
        session_handler,
        session_timeout_seconds: int = DEFAULT_fui_SESSION_TIMEOUT,
        oauth_state_timeout_seconds: int = DEFAULT_fui_OAUTH_STATE_TIMEOUT,
        upload_endpoint_path: Optional[str] = None,
        secret_key: Optional[str] = None,
    ):
        """
        Handle fui app WebSocket connections.
        ...
        * `secret_key` (str, optional) - secret key to sign upload requests.
        ...
        logger.info(f"New fuiApp: {self.__id}")
        ...
        env_session_timeout_seconds = os.getenv("FUI_SESSION_TIMEOUT")
        if env_session_timeout_seconds:
            self.__session_timeout_seconds = int(env_session_timeout_seconds)

        env_oauth_state_timeout_seconds = os.getenv("FUI_OAUTH_STATE_TIMEOUT")
    ...
    def dispose(self):
        logger.info(f"Disposing fuiApp: {self.__id}")
        self.__page = None
