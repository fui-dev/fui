...
from fui_web.fastapi.fui_app import (
    DEFAULT_fui_OAUTH_STATE_TIMEOUT,
    DEFAULT_fui_SESSION_TIMEOUT,
    fuiApp,
)
from fui_web.fastapi.fui_fastapi import FastAPI
from fui_web.fastapi.fui_oauth import fuiOAuth
from fui_web.fastapi.fui_static_files import fuiStaticFiles
from fui_web.fastapi.fui_upload import fuiUpload
...
def app(
    ...
    """
    Mount all fui FastAPI handlers in one call.
    ...
    * `oauth_state_timeout_seconds` (int, optional) - OAuth state lifetime, in seconds, which is a maximum allowed time between starting OAuth flow and redirecting to OAuth callback URL.
    ...
    env_upload_dir = os.getenv("FUI_UPLOAD_DIR")
    if env_upload_dir:
        upload_dir = env_upload_dir

    env_websocket_endpoint = os.getenv("FUI_WEBSOCKET_HANDLER_ENDPOINT")
    websocket_endpoint = (
        "ws" if not env_websocket_endpoint else env_websocket_endpoint.strip("/")
    )

    env_upload_endpoint = os.getenv("FUI_UPLOAD_HANDLER_ENDPOINT")
    upload_endpoint = (
        "upload" if not env_upload_endpoint else env_upload_endpoint.strip("/")
    )

    env_oauth_callback_endpoint = os.getenv("FUI_OAUTH_CALLBACK_HANDLER_ENDPOINT")
    ...
    async def oauth_redirect_handler(request: Request):
        return await fuiOAuth().handle(request)
