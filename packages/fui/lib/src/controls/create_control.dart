...
import 'package:fui/src/utils/badge.dart';
    ...
    builder: (context, controlView) {
      ...
      if (key != "") {
        if (key.startsWith("test:")) {
          ...
          fuiAppServices.of(context).globalKeys[key] = globalKey;
        }
      }
      ...
      for (var createControlFactory
          in fuiAppServices.of(context).createControlFactories) {
        widget = createControlFactory(CreateControlArgs(
            ...
            parentDisabled,
            parentAdaptive,
            fuiAppServices.of(context).server));
        if (widget != null) {
          break;
        }
      }

      // try creating fui built-in widget
      widget ??= createWidget(controlKey, controlView, parent, parentDisabled,
          parentAdaptive, nextChild, fuiAppServices.of(context).server);
...
Widget _rotatedControl(
  ...
  if (animation != null) {
    return AnimatedRotation(
        ...
        onEnd: control.attrBool("onAnimationEnd", false)!
            ? () {
                fuiAppServices.of(context).server.triggerControlEvent(
