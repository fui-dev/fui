...
String getWebsocketEndpointPath(String uriPath) {
  var meta = html.document.head
      ?.querySelector("meta[name='fui-websocket-endpoint-path']");
  return trim(meta?.attributes["content"] ?? "ws", "/");
}

String getfuiRouteUrlStrategy() {
  var meta =
      html.document.head?.querySelector("meta[name='fui-route-url-strategy']");
  return meta != null ? meta.attributes["content"]! : "";
}

bool isfuiWebPyodideMode() {
  var meta = html.document.head?.querySelector("meta[name='fui-web-pyodide']");
  return meta != null
