...
class fuiServer implements fuiControlBackend {
  ...
  fuiServer(this._store, this.controlInvokeMethods,
      {this.reconnectIntervalMs, this.reconnectTimeoutMs, this.errorsHandler});

  Future connect({required String address}) async {
    _address = address;

    debugPrint("Connecting to fui server $address...");
    try {
      _clientProtocol = fuiServerProtocol(
          address: _address,
      ...
      debugPrint("Error connecting to fui server: $e");
      _onDisconnect();
    }
  }
  ...
  _onDisconnect() {
    ...
    if (reconnectTimeoutMs == null ||
      ...
      errorsHandler
          ?.onError("Error connecting to a fui service in a timely manner.");
    }
  }
  ...
  void disconnect() {
    debugPrint("Disconnecting from fui server.");
    _disposed = true;
